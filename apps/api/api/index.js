...
import { createOrGetShareToken, getReceiptByToken } from '../lib/receipt-shares.js';
...
fastify.get('/api/bank-admin/dashboard', async (request, reply) => {
  try {
    ...
    return reply.code(200).send({
      ...
      daily_metrics: validatedDailyMetrics,
      confidence_counts: {
...
fastify.get('/api/receipts/:id', async (request, reply) => {
  try {
    ...
    if (isBelowThreshold) {
      // Log receipt_view_blocked event with BELOW_THRESHOLD reason
      logReceiptViewBlocked(receiptId, {
      ...
      // Return receipt but mark as below threshold and hide items
      return reply.code(200).send({
        success: true,
        receipt: {
          ...receipt,
          below_threshold: true,
          receipt_items: [] // Hide items
        },
        confidence_threshold: threshold
      });
    }
